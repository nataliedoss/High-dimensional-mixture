...
class ModelGM_HD:
    """ 
    class for d-dim Gaussian mixture model
    weights: ndarray of weights
    centers: ndarray of centers, of dimension 
    sigma: standard deviations, fixed at 1 
    """
    
    def __init__(self, w, x, std):
        """ 
        Initialize a GM model
        w: Array(float, k x 1). Entries are the mixing distribution weights. 
    ...
    def __repr__(self):
        return "atom: %s\nwght: %s\nsigm: %s" % (self.centers, self.weights, self.sigma)
...
def sample_gm(model, k, num, d):
    """
    Args:
    model: ModelGM_HD (with attributes centers, weights, sigma).
    k: Int. Estimated number of centers.
    num: Int. Number of samples in data.
