...
# In this code, k is the true number of components in the model while k_est is our guess of the number of components. This is different from the paper notation. The correspondence is:
# Code: k  Paper: k_0
# Code: k_est  Paper: k
...
def sim_over_n(num_sims, k, k_est, ld_est, d, factor_model, sigma, n_range,
    ...
    for i in range(len(n_range)):
        ...
        for j in range(num_sims):
            ...
            start_em = time.time()
            p, mu = em(sample, k_est, sigma, max_iter_EM, tol_EM)
...
max_iter_EM = 1000
tol_EM = .000001
name = "k" + str(k) + "_kest" + str(k_est) + "_d" + str(d) + "_factormodel" + str(factor_model) + "_weightseven" + "_sigma" + str(sigma) +  "_factorweights" + str(factor_weights) +  "_factorthetas" + str(factor_thetas) 
...
# Run sim
random.seed(3)
sim = sim_over_n(num_sims=num_sims, k=k, k_est=k_est, ld_est=ld_est,
                 d=d, factor_model=factor_model, 
